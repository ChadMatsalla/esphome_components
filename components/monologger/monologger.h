...
#endif
#ifdef USE_MONOLOGGER_USB_CDC
  UART_SELECTION_USB_CDC,
#endif
#ifdef USE_MONOLOGGER_USB_SERIAL_JTAG
...
class Monologger : public Component {
 public:
  explicit Monologger(uint32_t baud_rate, size_t tx_buffer_size);
#ifdef USE_MONOLOGGER_USB_CDC
  void loop() override;
#endif
...
extern Monologger *global_logger;  // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)

class MonologgerMessageTrigger : public Trigger<int, const char *, const char *> {
 public:
  explicit MonologgerMessageTrigger(Monologger *parent, int level) {
    this->level_ = level;
