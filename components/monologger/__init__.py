...
monologger_ns = cg.esphome_ns.namespace("monologger")
LOG_LEVELS = {
...
CONF_MONOLOGGER_ID = "monologger_id"
...
HARDWARE_UART_TO_UART_SELECTION = {
    UART0: monologger_ns.UART_SELECTION_UART0,
    UART0_SWAP: monologger_ns.UART_SELECTION_UART0_SWAP,
    UART1: monologger_ns.UART_SELECTION_UART1,
    UART2: monologger_ns.UART_SELECTION_UART2,
    USB_CDC: monologger_ns.UART_SELECTION_USB_CDC,
    USB_SERIAL_JTAG: monologger_ns.UART_SELECTION_USB_SERIAL_JTAG,
    DEFAULT: monologger_ns.UART_SELECTION_DEFAULT,
}
...
Monologger = monologger_ns.class_("Monologger", cg.Component)
MonologgerMessageTrigger = monologger_ns.class_(
    "MonologgerMessageTrigger",
...
CONFIG_SCHEMA = cv.All(
    cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(Monologger),
            ...
            cv.Optional(CONF_ON_MESSAGE): automation.validate_automation(
                {
                    cv.GenerateID(CONF_TRIGGER_ID): cv.declare_id(MonologgerMessageTrigger),
...
async def to_code(config):
    ...
    try:
        ...
        cg.add_define("USE_MONOLOGGER_USB_SERIAL_JTAG")
    except cv.Invalid:
        pass
    try:
        uart_selection(USB_CDC)
        cg.add_define("USE_MONOLOGGER_USB_CDC")
    except cv.Invalid:
        pass
...
CONF_MONOLOGGER_LOG = "monologger.log"
MONOLOGGER_LOG_ACTION_SCHEMA = cv.All(
...
@automation.register_action(CONF_MONOLOGGER_LOG, LambdaAction, MONOLOGGER_LOG_ACTION_SCHEMA)
async def monologger_log_action_to_code(config, action_id, template_arg, args):
...
@automation.register_action(
    "monologger.set_level",
    LambdaAction,
    cv.maybe_simple_value(
        {
            cv.GenerateID(CONF_MONOLOGGER_ID): cv.use_id(Monologger),
...
async def monologger_set_level_to_code(config, action_id, template_arg, args):
    level = LOG_LEVELS[config[CONF_LEVEL]]
    monologger = await cg.get_variable(config[CONF_MONOLOGGER_ID])
    if tag := config.get(CONF_TAG):
        text = str(cg.statement(monologger.set_log_level(tag, level)))
    else:
        text = str(cg.statement(monologger.set_log_level(level)))
